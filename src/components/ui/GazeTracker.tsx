...
const SECTION_PROPS: Record<SectionId, { emoji: string; label: string; position: string }> = {
    welcome:   { emoji: "\u{1F44B}", label: "wave",       position: "-top-4 -right-2" },
    work:      { emoji: "\u{2615}",  label: "coffee",     position: "-top-3 -right-3" },
    projects:  { emoji: "\u{1F9EA}", label: "experiment", position: "-top-4 -right-1" },
    education: { emoji: "\u{1F4DA}", label: "books",      position: "-top-3 -right-2" },
};
...
export default function GazeTracker({
    ...
    return (
      <div
        className={cn(`relative w-full h-full bg-transparent overflow-visible`, className)}
      >
          {currentImage && (
